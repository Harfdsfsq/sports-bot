...
def poisson_over_probability(total_lambda: float, point: float) -> float:
    return over_probability_from_lambda(total_lambda, point)
...
def russian_selection(family: str, selection: str, point: float | None = None) -> str:
    ...
    if family_key == 'totals':
        ...
        if low.startswith('under'):
            return 'Меньше'
    if family_key == 'btts':
        if low in {'yes', 'both', 'btts_yes'}:
            return 'Да'
        if low in {'no', 'btts_no'}:
            return 'Нет'
    if family_key == 'h2h':
        if low in {'draw', 'x'}:
            return 'Ничья'
