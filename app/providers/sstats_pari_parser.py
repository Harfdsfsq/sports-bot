...
def extract_odds(payload: Any) -> list[dict[str, Any]]:
    data = unwrap(payload)
    if isinstance(data, dict):
        for key in ("odds", "Odds", "coefficients"):
            value = data.get(key)
            if isinstance(value, list):
                return [row for row in value if isinstance(row, dict)]
    return []
        ...
        for key in ("totalCount", "total_count", "total"):
            if key in current:
                try:
                    ...
                    pass
        current = next((current[key] for key in ("data", "result", "response") if isinstance(current.get(key), dict)), None)
        if current is None:
            break
    return fallback
...
def parse_market_name(name: str, match: Match) -> tuple[str, str, float | None, str | None, str] | None:
    ...
    if any(token in text for token in ("total", "тотал", "goals over", "goals under")) and (is_over or is_under):
    ...
    if any(token in text for token in ("handicap", "spread", "фора")):
...
def parse_offers(rows: list[dict[str, Any]], match: Match, source_event_id: str) -> list[Offer]:
    ...
    for row in rows:
        ...
        seen.add(identity)
        offers.append(Offer(
            source="sstats_pari", bookmaker="Pari", family=family, selection=selection,
            price=price, point=point, team_side=side, market_name=name,
            market_key=market_key, source_event_id=source_event_id,
            metadata={
                "provider": "sstats_pari", "independent_odds_source": True,
                "raw_outcome_id": row.get("id") or row.get("outcomeId"),
                "raw_name": name, "fetched_at_utc": datetime.now(UTC).isoformat(),
            },
        ))
    return offers
