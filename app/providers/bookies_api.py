...
class BookiesApiProvider:
        ...
        async with httpx.AsyncClient(timeout=self.settings.bookies_api_timeout_seconds) as client:
            for day_key, day_matches in sorted(candidates_by_date.items()):
                for page in range(1, max(1, self.settings.bookies_api_max_pages_per_day) + 1):
                    params = self._auth_params("predatapage", {
                    ...
                    for raw in items:
                        ...
                        if matched is None:
                            ...
                            continue
                        matched_events[matched.match_key] = {"match": matched, "event": event, "raw": raw}
                        matched_this_page += 1
            ...
            for match_key, item in matched_events.items():
                ...
                if not parsed_offers and self.settings.bookies_api_odds_task != "odds":
                    parsed_offers, odds_payload = await self._fetch_odds_for_game(client, game_id, item["match"], stats, task="odds")
                    if odds_payload is not None and len(preview["sample_odds"]) < 2:
                        preview["sample_odds"].append(odds_payload)
                ...
                stats["markets_parsed"] += len({(o.bookmaker, o.family, o.market_name, o.point) for o in parsed_offers})
    ...
    def _candidate_matches(self, matches: list[Match], existing_offer_maps: dict[str, dict[str, list[Offer]]]) -> list[Match]:
    ...
    def _parse_event(self, raw: Any) -> dict[str, Any] | None:
        if not isinstance(raw, dict):
            return None
        home = self._first_text(raw, ["home", "home_team", "team1", "localteam", "homeTeam", "ht"])
        away = self._first_text(raw, ["away", "away_team", "team2", "visitorteam", "awayTeam", "at"])
        if not home or not away:
            teams = raw.get("teams")
            if isinstance(teams, list) and len(teams) >= 2:
                home = home or self._first_text(teams[0], ["name", "team", "title"])
                away = away or self._first_text(teams[1], ["name", "team", "title"])
        if not home or not away:
            return None
        game_id = self._first_text(raw, ["game_id", "gameId", "id", "match_id", "event_id"])
        league = self._first_text(raw, ["league", "league_name", "competition", "tournament", "championship", "sport_title"])
        date_value = None
    ...
    async def _fetch_odds_for_game(
        ...
        stats["payload_shapes"].append(self._payload_shape(payload))
        stats["last_body_preview"] = response.text[:1200]
        return self._parse_odds_payload(payload, match), payload
    ...
    def _parse_odds_payload(self, payload: Any, match: Match) -> list[Offer]:
        ...
        def parse_selection_item(item: dict[str, Any], ctx: dict[str, Any]) -> None:
            ...
            if family == "h2h":
                ...
                if total_key and point is not None:
                    add_offer(bookmaker, "totals", "Over" if total_key == "over" else "Under", price, point=point, market_name=market_name, market_key=market_key)
            elif family == "spreads":
                ...
                mapping = {"1x": "1X", "x2": "X2", "12": "12", "homedraw": "1X", "awaydraw": "X2", "homeaway": "12"}
                if raw in mapping:
                ...
                if total_key and side and point is not None:
                    add_offer(bookmaker, "teamTotals", "Over" if total_key == "over" else "Under", price, point=point, market_name=market_name, market_key=market_key, team_side=side)
        ...
        def parse_scalar_mapping(node: dict[str, Any], ctx: dict[str, Any]) -> None:
            ...
            for key, value in node.items():
                ...
                if price is None or price <= 1.0:
                    continue
                parse_selection_item({"name": key, "price": price}, {**ctx, "point": point, "market_name": market_name, "market_key": market_key, "bookmaker": bookmaker})
