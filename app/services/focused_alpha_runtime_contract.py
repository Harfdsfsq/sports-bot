...
def _start_lifecycle_once() -> dict[str, Any]:
    """Delete stale debug only once per GitHub run, including across processes."""
    ...
    if existing and _same_run(existing):
        ...
        return {
            "status": "already_started_same_github_run",
            "started_at_utc": started or None,
            "github_run_id": existing.get("github_run_id"),
            "github_run_attempt": existing.get("github_run_attempt"),
            "lifecycle_status": existing.get("status"),
            "stale_debug_removed": bool(existing.get("stale_debug_removed")),
        }
...
def _relax_discovery_reuse(target: Any, result: Any) -> dict[str, Any]:
    ...
    if not mode or "incremental" in mode or not status.startswith("ok"):
        return result
    if age < 0 or age > refresh:
        return result
    ...
    floor = max(
        ...
        min(
            nominal,
            int(target.env_int("RUNBOT_DISCOVERY_FIRST_REUSE_MIN_INVENTORY_ROWS", min(260, nominal))),
        ),
    )
    current = max(0, int(target.inventory_matches()))
    result.update(
        current_inventory_matches=current,
        reuse_min_inventory_rows=floor,
        nominal_inventory_target=nominal,
    )
    if current >= floor:
        result.update(
            reusable=True,
            reuse_reason="fresh_full_checkpoint_inventory_floor",
            inventory_topup_required=current < nominal,
        )
    return result
...
def _reassert_preflight_policy() -> bool:
    try:
        ...
        runtime_preflight.DISCOVERY_FIRST_DEFAULTS.update(
            {key: value for key, value in POLICY.items() if key.startswith("RUNBOT_")}
        )
        return True
    except Exception:
        return False
...
def apply(*, force: bool = True) -> dict[str, Any]:
    ...
    payload = {
        ...
        "mode": "focused_alpha_rules_ab_publish",
        ...
        "main_publication_disabled_for_wide_data_window": False,
        "live_learning_auto_tuning": False,
        "preflight_policy_reasserted": preflight_reasserted,
        "discovery_reuse_floor_patch": reuse_patch,
