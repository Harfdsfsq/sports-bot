...
CORE_ODDS_SOURCES = {"odds_api_io", "sportlogic", "bzzoiro", "allsportsapi", "oddspapi", "bookies_api", "sstats"}
CORE_CONTEXT_SOURCES = {"sportlogic", "sstats", "bzzoiro", "football_data", "api_football", "futrixmetrics", "self_history"}
LOW_PRIORITY_CONTEXT_PROVIDERS = {"weather", "newsapi", "gnews"}
...
def _patch_day_inventory() -> dict[str, Any]:
    ...
    def build_payload_rules(self: Any, *, local_date: str, matches: list[Any], source_meta: dict[str, Any] | None = None, existing: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        payload["rules_policy"] = {
            "enabled": True,
            "top_inventory_limit": max_matches,
            "time_buckets": BUCKET_ORDER,
            "ranking": "nearest_window_league_coverage_score",
        }
        return payload
...
def _patch_runner() -> dict[str, Any]:
    ...
    def select_publishable_rules(self: Any, candidates: list[Any]) -> list[Any]:
        ...
        ranked = sorted(
            candidates,
            key=lambda c: (
                _bucket_rank(str(_candidate_get(c, "time_bucket") or (_candidate_get(c, "source_summary", {}) or {}).get("time_bucket") or "24h+")),
                -_candidate_value_score(c),
            ),
        )
