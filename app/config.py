...
class Settings(BaseSettings):
    ...
    storage_export_dir: str = Field(
        ...
        validation_alias=AliasChoices("STORAGE_EXPORT_DIR", "EXPORT_DIR"),
    )
    ...
    min_model_confidence: float = Field(default=55.0, validation_alias=AliasChoices("MIN_MODEL_CONFIDENCE"))
    ...
    enable_odds_api_io: bool = Field(
        default=True,
        validation_alias=AliasChoices("ENABLE_ODDS_API_IO", "ODDS_API_IO_ENABLED"),
    )
    ...
    @field_validator("min_model_confidence", mode="before")
    @classmethod
    def normalize_model_confidence(cls, value: Any) -> Any:
        if value is None:
            return 55.0
        try:
            number = float(value)
