...
POLICY_VERSION = 'v1-capacity-keypool-shortlist'

# Technical key pools. These do not bypass provider terms: if a provider returns
# 429 or explicit limit error, that key/provider is cooled down by the existing
# budget/cooldown layer. Multiple keys are only safe when the provider account
# rules allow multiple independent keys/accounts.
KEY_POOL_ENV = {
...
PROVIDER_PATCHES: dict[str, dict[str, Any]] = {
    'odds_api_io': {
        'enabled': True,
        'per_run_max': 100,
        'limit': {'requests_per_hour': 100, 'budget_scope': 'per_run', 'bookmakers': 2},
        'env': {
            ...
            # Keep 2 bookmakers because the plan card says 2 bookmakers. More bookmakers
            # can multiply request cost and should only be enabled after plan upgrade.
    ...
    'weatherapi': {
        'enabled': True,
        'per_run_max': 40,
        'env': {'WEATHERAPI_PER_RUN_MAX': '40', 'WEATHERAPI_MAX_HTTP_REQUESTS_PER_RUN': '40', 'WEATHER_CONTEXT_MATCH_LIMIT': '80', 'WEATHER_CACHE_TTL_MINUTES': '240'},
    },
    'openweathermap': {
        'enabled': True,
        'per_run_max': 24,
        'env': {'OPENWEATHERMAP_PER_RUN_MAX': '24', 'OPENWEATHERMAP_MAX_HTTP_REQUESTS_PER_RUN': '24', 'WEATHER_OPENWEATHERMAP_FALLBACK_ENABLED': 'true'},
    },
    'football_data': {
        'enabled': True,
        'per_run_max': 16,
        'env': {'FOOTBALL_DATA_PER_RUN_MAX': '16', 'FOOTBALL_DATA_REQUESTS_MAX_PER_RUN': '16', 'FOOTBALL_DATA_CONTEXT_MATCH_LIMIT': '180'},
    },
    'thesportsdb': {
        'enabled': True,
        'per_run_max': 30,
        'env': {'THESPORTSDB_PER_RUN_MAX': '30', 'THESPORTSDB_REQUESTS_MAX_PER_RUN': '30', 'THESPORTSDB_CONTEXT_MATCH_LIMIT': '180'},
    },
}
...
def main() -> int:
    ...
    policy['description'] = 'Per-run capacity policy with shortlist-first source expansion and technical key-pool env discovery.'
    notes = list(policy.get('notes') or []) if isinstance(policy.get('notes'), list) else []
    notes.append('odds-api.io is set to 100/run because scheduled runs are every 2h and the dashboard plan indicates 100 requests/hour. Keep 2 bookmakers on the free plan.')
    notes.append('SStats is set to 150/run according to the requested per-run target; provider 429 responses must still trigger cooldown.')
    notes.append('External signals are added as confirmation context only, not standalone predictors.')
    policy['notes'] = notes
