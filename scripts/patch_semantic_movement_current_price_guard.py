...
_OFFER_INDEX_CACHE: tuple[str, dict[tuple[str, str, str, str], list[dict[str, Any]]]] | None = None
_LINE_INDEX_CACHE: tuple[str, dict[tuple[str, str, str, str], list[dict[str, Any]]]] | None = None
_INSTALLED = False
_ORIGINAL_HARD_REJECT = None
_ORIGINAL_CANONICAL_PUBLICATION_KEY = None
_ORIGINAL_SELECT_TOP_PICKS = None

_STOP = {"fc", "cf", "fk", "sc", "afc", "ac", "club", "football", "futbol", "calcio", "msk", "tc", "sk", "sv", "nk", "rks", "pfc", "fkc", "ks", "as", "us", "cd"}
...
def _team_similarity(left: Any, right: Any) -> float:
    ...
    if left_compact == right_compact:
        return 1.0
    if min(len(left_compact), len(right_compact)) >= 4 and (left_compact in right_compact or right_compact in left_compact):
        return 0.96
    compact_score = SequenceMatcher(None, left_compact, right_compact).ratio()
    left_scores = [max(SequenceMatcher(None, token, other).ratio() for other in right_tokens) for token in left_tokens]
    right_scores = [max(SequenceMatcher(None, token, other).ratio() for other in left_tokens) for token in right_tokens]
    token_score = (sum(left_scores) / len(left_scores) + sum(right_scores) / len(right_scores)) / 2.0
    return max(compact_score, token_score, SequenceMatcher(None, left_text, right_text).ratio())
...
def _fixture(row: dict[str, Any]) -> tuple[str, str, str, datetime | None]:
    ...
    home = str(row.get("home_team") or row.get("home") or row.get("home_name") or key_home)
    away = str(row.get("away_team") or row.get("away") or row.get("away_name") or key_away)
...
def _same_fixture(left: dict[str, Any], right: dict[str, Any]) -> bool:
    ...
    if left_day and right_day and left_day != right_day:
        return False
    if left_time is not None and right_time is not None and abs((left_time - right_time).total_seconds()) > 3 * 3600:
        return False
    same_order = (_team_similarity(left_home, right_home), _team_similarity(left_away, right_away))
    reversed_order = (_team_similarity(left_home, right_away), _team_similarity(left_away, right_home))
...
def _family(row: dict[str, Any]) -> str:
    ...
    if "teamtotal" in compact or "individualtotal" in compact or "индивидуальн" in text or ("team" in text.split() and "total" in text.split()):
        return "team_totals"
...
def _side(row: dict[str, Any]) -> str:
    text = _ascii(" ".join(str(row.get(key) or "") for key in ("selection_key", "selection", "side", "name", "outcome")))
...
def _same_market(left: dict[str, Any], right: dict[str, Any]) -> bool:
    ...
    return left_point is not None and right_point is not None and abs(left_point - right_point) <= 1e-6
...
def _offer_index(payload: dict[str, Any], path: Path | None) -> dict[tuple[str, str, str, str], list[dict[str, Any]]]:
...
def _matching_offers(candidate: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    max_age = max(5.0, _env_float("CONTROLLED_FALLBACK_CURRENT_OFFER_MAX_AGE_MINUTES", 180.0))
    stale = created is None or datetime.now(UTC) - created > timedelta(minutes=max_age)
    return matches, {"path": str(path) if path else None, "created_at_utc": created.isoformat() if created else None, "stale": stale, "matching_offers": len(matches)}
    ...
    for path in LINE_HISTORY_PATHS:
        ...
        for key, box in lines.items():
            if not isinstance(box, dict):
                continue
            snapshot = box.get("last_snapshot") if isinstance(box.get("last_snapshot"), dict) else {}
            probe = dict(snapshot)
            probe.setdefault("match_key", str(key).split("|totals|")[0] if "|totals|" in str(key) else str(key).split("|spreads|")[0])
            guard = box.get("last_guard") if isinstance(box.get("last_guard"), dict) else {}
            item = {"key": key, "snapshot": snapshot, "probe": probe, "guard": guard, "path": str(path)}
...
def _line_entries(candidate: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    selected_book = _book(candidate.get("bookmaker") or summary.get("selected_bookmaker"))
    ...
    for item in _line_index().get(_market_index_key(candidate), []):
        ...
        book = _book(snapshot.get("bookmaker") or str(item["key"]).rsplit("|", 1)[-1])
...
def _requires_current_snapshot(candidate: dict[str, Any]) -> bool:
    summary = candidate.get("source_summary") if isinstance(candidate.get("source_summary"), dict) else {}
    text = _ascii(" ".join(str(value or "") for value in (candidate.get("_candidate_source"), candidate.get("candidate_source"), candidate.get("source"), candidate.get("bookmaker"), summary.get("selected_source"), summary.get("selected_bookmaker"))))
    markers = ("odds api io", "a cover market promotion", "b cover market promotion", "betfair", "bet365", "unibet", "sbobet")
    return any(marker in text for marker in markers)


def _best_current_offer(candidate: dict[str, Any], offers: list[dict[str, Any]]) -> dict[str, Any] | None:
    if not offers:
        return None
    selected = _price(candidate)
    if selected <= 1.0:
        return max(offers, key=_price)
    # Prefer the closest current quote to the selected price. This repairs stale
    # selected-book labels while preserving price integrity on the same fixture,
    # market, side and point.
    return min(offers, key=lambda row: abs(_price(row) - selected))
...
def semantic_integrity_reasons(candidate: dict[str, Any], metrics: dict[str, Any] | None = None) -> list[str]:
    ...
    require_price = _env_bool("CONTROLLED_FALLBACK_REQUIRE_FRESH_SELECTED_PRICE", True) and _requires_current_snapshot(candidate)
    allow_book_repair = _env_bool("CONTROLLED_FALLBACK_ALLOW_CURRENT_BOOK_SUBSTITUTION", True)
    if offer_diagnostics["stale"]:
        ...
        summary = candidate.get("source_summary") if isinstance(candidate.get("source_summary"), dict) else {}
        selected_book = _book(candidate.get("bookmaker") or summary.get("selected_bookmaker"))
        book_offers = [row for row in offers if not selected_book or _book(row.get("bookmaker") or row.get("book")) == selected_book]
        selected = _price(candidate)
        selected_source = book_offers if book_offers else offers
        current = max(selected_source, key=_price) if book_offers else _best_current_offer(candidate, offers)
        current_price = _price(current or {})
        if selected_book and not book_offers and not allow_book_repair:
            reasons.append("semantic_selected_book_current_price_missing")
        if current and selected > 1.0 and current_price > 1.0:
            absolute_tolerance = max(0.0, _env_float("CONTROLLED_FALLBACK_CURRENT_PRICE_ABS_TOLERANCE", 0.05))
            percent_tolerance = max(0.0, _env_float("CONTROLLED_FALLBACK_CURRENT_PRICE_PCT_TOLERANCE", 2.5))
            difference = abs(selected - current_price)
            difference_pct = difference / current_price * 100.0 if current_price > 0 else 999.0
            ...
            offer_diagnostics.update({"selected_bookmaker": selected_book, "selected_price": round(selected, 4), "selected_book_missing_from_current_snapshot": bool(selected_book and not book_offers), "selected_book_repaired_with_current_book": bool(selected_book and not book_offers and allow_book_repair), "current_price": round(current_price, 4), "current_bookmaker": current.get("bookmaker") or current.get("book"), "price_diff_pct": round(difference_pct, 3)})
    elif require_price:
        reasons.append("semantic_current_exact_market_price_missing")

    entries = _line_entries(candidate)
    movement_diagnostics: dict[str, Any] = {"matching_entries": len(entries), "entries": []}
    require_movement = _env_bool("PUBLISH_REQUIRE_LINE_MOVEMENT", True) and _env_bool("CONTROLLED_FALLBACK_REQUIRE_SEMANTIC_LINE_MOVEMENT", True)
    if entries:
        times = [_parse_dt(item["snapshot"].get("captured_at_utc")) for item in entries]
        ...
        for item in recent:
            ...
            is_passed = bool(guard.get("passed")) and status in {"movement_confirmed", "publish_now_no_next_cron", "confirmed", "passed"}
            is_failed = status in {"movement_failed", "failed"}
            no_more = bool(guard.get("no_more_cron_before_kickoff")) or bool(guard.get("final_pre_kickoff_check"))
            if no_more and not bool(guard.get("passed")) and status in {"awaiting_next_run", "pending", ""}:
                unresolved_final = True
            passed = passed or is_passed
            failed = failed or is_failed
            movement_diagnostics["entries"].append({"key": item["key"], "captured_at_utc": item["snapshot"].get("captured_at_utc"), "status": status, "passed": bool(guard.get("passed")), "no_more_cron_before_kickoff": bool(guard.get("no_more_cron_before_kickoff")), "current_odds": guard.get("current_odds"), "line_move_pct": guard.get("line_move_pct"), "reasons": guard.get("reasons") or []})
        if passed and failed:
            reasons.append("semantic_line_movement_alias_conflict")
        elif failed:
            reasons.append("semantic_line_movement_failed")
        elif any(str(item.get("guard", {}).get("line_movement_lifecycle_status") or "").strip().lower() == "not_publishable" for item in recent):
            ...
            reasons.append(f"semantic_line_movement_not_publishable:{blocked_reasons[0]}" if blocked_reasons else "semantic_line_movement_not_publishable")
...
def sanitize_rescue_pool() -> dict[str, Any]:
    ...
    for row in rows:
        ...
        semantic_reasons = [reason for reason in reasons if reason.startswith("semantic_")]
        if semantic_reasons:
            removed.append({"match_key": row.get("match_key"), "home_team": row.get("home_team"), "away_team": row.get("away_team"), "family": row.get("family"), "selection": row.get("selection"), "point": row.get("point"), "odds": row.get("odds"), "reasons": semantic_reasons, "diagnostics": diagnostics})
        else:
            kept.append(row)
    if rows:
        output: Any = kept if container is None else dict(payload)
        if container is not None:
            output[container] = kept
        RESCUE_PATH.parent.mkdir(parents=True, exist_ok=True)
        RESCUE_PATH.write_text(json.dumps(output, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        _CACHE.pop(str(RESCUE_PATH), None)
    result = {"status": "ok", "created_at_utc": datetime.now(UTC).isoformat(), "rescue_rows_seen": len(rows), "rescue_rows_kept": len(kept), "rescue_rows_removed": len(removed), "removed_sample": removed[:20], "publication_contract_relaxed": False}
    _write(result)
    return result
...
def install(base: Any) -> dict[str, Any]:
    global _INSTALLED, _ORIGINAL_HARD_REJECT, _ORIGINAL_CANONICAL_PUBLICATION_KEY, _ORIGINAL_SELECT_TOP_PICKS
    ...
    if callable(current_select):
        ...
        def select_top_picks(viable: Any, bankroll: dict[str, Any]):
            ...
            for item in selected:
                ...
                key = _semantic_fixture_key(candidate) if isinstance(candidate, dict) else str(candidate)
                if key in seen:
                    continue
                seen.add(key)
                result.append(item)
    ...
    _INSTALLED = True
    result = {"status": "installed", "created_at_utc": datetime.now(UTC).isoformat(), "hard_reject_patched": callable(current_hard), "canonical_publication_key_patched": callable(current_key), "selected_bundle_semantic_dedupe_patched": callable(current_select), "sanitizer": sanitizer, "publication_contract_relaxed": False}
    _write(result)
    return result
