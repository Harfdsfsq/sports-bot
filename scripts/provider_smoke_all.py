...
USER_AGENT = os.getenv(
    "PROVIDER_SMOKE_USER_AGENT",
    "HARIZON-sports-bot-provider-smoke/1.0 (https://github.com/Harfdsfsq/sports-bot)",
)
...
def _missing_required_envs(names: tuple[str, ...]) -> list[str]:
    return [name for name in names if not str(os.getenv(name) or "").strip()]
...
def _status(http_status: int | None, rows_count: int, error: str | None, key_present: bool, body: str) -> tuple[str, str]:
    ...
    if error:
        if "Name or service not known" in error or "ConnectError" in error:
...
def _apply_key(probe: Probe) -> tuple[Probe, str, bool, list[str]]:
    missing_config = _missing_required_envs(probe.required_envs)
...
def _rapid_probe(
    ...
    return Probe(
        ...
        note=f"RapidAPI host={host}; override {host_env}/{path_env or 'path'} if endpoint differs",
    )
...
def _bookies_probe() -> Probe:
    ...
    if not base:
        return Probe(
            ...
            required_envs=("BOOKIES_API_BASE_URL",),
            note="Set BOOKIES_API_BASE_URL plus BOOKIES_API_KEY or BOOKIES_API_TOKEN.",
        )
    base = base.rstrip("/")
    ...
    return Probe(
        ...
        headers=headers,
        params=params,
        note="Configurable via BOOKIES_API_BASE_URL and BOOKIES_API_SMOKE_PATH.",
    )
...
def _sportlogic_probes(today: str, tomorrow: str) -> list[Probe]:
    ...
    return [
        Probe(
            "sportlogic_games_dated",
            "odds",
            f"{base}/games",
            key_envs=key_envs,
            headers=headers,
            params={"date_from": today, "date_to": tomorrow, "per_page": 5},
            note="dated games endpoint",
        ),
        Probe(
            ...
            headers=headers,
            params={"per_page": 5},
            note="broad games endpoint, used when dated query is empty",
        ),
        Probe(
            "sportlogic_leagues",
            "context",
...
def build_probes() -> list[Probe]:
    ...
    clubelo_base = _env("CLUBELO_BASE_URL", "http://api.clubelo.com").rstrip("/")
    highlightly_base = _env("HIGHLIGHTLY_BASE_URL").rstrip("/")
    highlightly_url = (highlightly_base + _env("HIGHLIGHTLY_SMOKE_PATH", "/football/leagues")) if highlightly_base else "about:blank"
    ...
    probes: list[Probe] = [
        Probe(
            ...
            key_envs=("ODDS_API_IO_KEY",),
            params={"apiKey": "${KEY}", "sport": "football", "status": "pending,live", "limit": 3},
            note="events bootstrap",
        ),
        Probe(
            "odds_api_io_account2",
            "odds",
            "https://api.odds-api.io/v3/events",
            key_envs=("ODDS_API_IO_KEY_2", "ODDS_API_IO_KEY2"),
            params={"apiKey": "${KEY}", "sport": "football", "status": "pending,live", "limit": 3},
            note="second odds-api.io account",
        ),
        _bookies_probe(),
        _rapid_probe(
            "oddspapi_rapidapi",
            "ODDSPAPI_RAPIDAPI_HOST",
            "oddspapi.p.rapidapi.com",
            ("ODDSPAPI_API_KEY", "ODDS_PAPI_API_KEY", "RAPIDAPI_KEY"),
            path="/sports",
            path_env="ODDSPAPI_RAPIDAPI_PATH",
            group="odds",
        ),
        _rapid_probe(
            "oddsfeed_rapidapi",
            ...
            path="/sports",
            path_env="ODDS_FEED_RAPIDAPI_PATH",
            group="odds",
        ),
        _rapid_probe(
            "sportsbook_rapidapi",
            ...
            ("SPORTSBOOK_RAPIDAPI_KEY", "BOOKIES_API_KEY", "RAPIDAPI_KEY"),
            path="/sports",
            path_env="SPORTSBOOK_RAPIDAPI_PATH",
            group="odds",
        ),
        Probe(
            "bzzoiro_events",
            ...
            params={"date_from": today, "date_to": tomorrow, "tz": "UTC", "page": 1},
        ...
        Probe(
            ...
            key_envs=("BZZOIRO_API_KEY",),
            headers={"Authorization": "Token ${KEY}"},
            params={"upcoming": "true", "date_from": today, "date_to": tomorrow, "tz": "UTC", "page": 1},
            note="predictions endpoint",
        ),
        Probe(
            "sstats",
            "context",
            "https://api.sstats.net/Games/list",
            key_envs=("SSTATS_API_KEY",),
            params={"apikey": "${KEY}", "from": today, "to": tomorrow, "limit": 5, "offset": 0},
            note="games list",
        ),
        Probe(
            "api_football",
            ...
            note="API-Sports status",
        ),
        Probe(
            "allsportsapi",
            "context",
            "https://apiv2.allsportsapi.com/football/",
            key_envs=("ALLSPORTSAPI_API_KEY",),
            params={"met": "Leagues", "APIkey": "${KEY}"},
            note="leagues endpoint; free may be limited",
        ),
        Probe(
            "football_data",
            "context",
            "https://api.football-data.org/v4/competitions",
            key_envs=("FOOTBALL_DATA_API_KEY", "FOOTBALL_DATA_KEY"),
            headers={"X-Auth-Token": "${KEY}"},
            note="competitions/free access",
        ),
        Probe(
            "thesportsdb",
            "context",
            "https://www.thesportsdb.com/api/v1/json/${KEY}/search_all_leagues.php",
            key_envs=("THESPORTSDB_API_KEY",),
            params={"s": "Soccer"},
            note="uses secret key or 123 if configured",
        ),
        Probe(
            "futrixmetrics",
            "context",
            _env("FUTRIXMETRICS_BASE_URL", "https://footballperformanceapi.site").rstrip("/") + "/database/ratings",
            key_envs=("FUTRIXMETRICS_API_KEY", "FUTRIXMETRICS_KEY"),
            headers={"X-API-Key": "${KEY}"},
            params={"team": "Arsenal", "league": "England - Premier League", "limit": 3},
            note="ratings endpoint",
        ),
        Probe(
            "highlightly",
            "context",
            ...
            headers={"x-api-key": "${KEY}"},
            note="Set HIGHLIGHTLY_BASE_URL/HIGHLIGHTLY_SMOKE_PATH; no safe default host is assumed.",
            required_envs=("HIGHLIGHTLY_BASE_URL",),
        ),
        _rapid_probe(
            "free_football_rapidapi",
            "FREE_FOOTBALL_RAPIDAPI_HOST",
            "free-api-live-football-data.p.rapidapi.com",
            ("FREE_FOOTBALL_RAPIDAPI_KEY", "RAPIDAPI_KEY"),
            path="/football-get-all-leagues",
            path_env="FREE_FOOTBALL_RAPIDAPI_PATH",
            group="context",
        ),
        _rapid_probe(
            "sportapi7_rapidapi",
            "SPORTAPI7_RAPIDAPI_HOST",
            "sportapi7.p.rapidapi.com",
            ("SPORTAPI7_RAPIDAPI_KEY", "RAPIDAPI_KEY"),
            path="/api/v1/sport/football/categories",
            path_env="SPORTAPI7_RAPIDAPI_PATH",
            group="context",
        ),
        Probe(
            "weatherapi",
            ...
            key_envs=("WEATHERAPI_KEY", "WEATHER_API_KEY", "WEATHERAPI_API_KEY"),
            params={"key": "${KEY}", "q": "London", "aqi": "no"},
        ...
        Probe(
            ...
            key_envs=("OPENWEATHERMAP_API_KEY", "OPENWEATHER_API_KEY", "OPENWEATHERMAP_KEY"),
            params={"appid": "${KEY}", "q": "London"},
            note="current weather",
        ),
        Probe(
            "open_meteo",
            ...
            note="no-key weather fallback",
        ),
        Probe(
            "meteostat_rapidapi",
            "weather",
            "https://meteostat.p.rapidapi.com/stations/nearby",
            key_envs=("METEOSTAT_RAPIDAPI_KEY", "RAPIDAPI_KEY"),
            headers={"x-rapidapi-key": "${KEY}", "x-rapidapi-host": _env("METEOSTAT_RAPIDAPI_HOST", "meteostat.p.rapidapi.com")},
            params={"lat": 51.5072, "lon": -0.1276, "limit": 1},
            note="RapidAPI Meteostat nearby station",
        ),
        Probe(
            "newsapi",
            "news",
            "https://newsapi.org/v2/top-headlines",
            key_envs=("NEWSAPI_KEY",),
            params={"apiKey": "${KEY}", "category": "sports", "pageSize": 3, "language": "en"},
            note="top headlines sports",
        ),
        Probe(
            "currents",
            "news",
            "https://api.currentsapi.services/v1/latest-news",
            key_envs=("CURRENTS_API_KEY", "CURRENTS_KEY"),
            params={"apiKey": "${KEY}", "category": "sports", "language": "en"},
            note="latest sports news",
        ),
        Probe(
            "gnews",
            "news",
            ...
            key_envs=("GNEWS_KEY",),
            params={"token": "${KEY}", "topic": "sports", "lang": "en", "max": 3},
            note="top headlines sports",
        ),
        Probe(
            "newsdata",
            ...
            key_envs=("NEWSDATA_API_KEY", "NEWSDATA_KEY"),
            params={"apikey": "${KEY}", "category": "sports", "language": "en"},
            note="sports news",
        ),
        Probe(
            ...
            key_envs=("GUARDIAN_API_KEY", "GUARDIAN_KEY"),
            params={"api-key": "${KEY}", "q": "football", "page-size": 3},
            note="Guardian search",
        ),
        Probe(
            "clubelo_today",
            "mapping",
            f"{clubelo_base}/{today}",
            required_secret=False,
            note="CSV club Elo ratings for today; CLUBELO_BASE_URL may override http default",
        ),
        Probe(
            "clubelo_team",
            "mapping",
            f"{clubelo_base}/Arsenal",
            required_secret=False,
            note="CSV club Elo team history",
        ),
        Probe(
            "football_data_co_uk_epl",
            "csv",
            f"https://www.football-data.co.uk/mmz4281/{season}/E0.csv",
            required_secret=False,
            note="CSV current EPL season; season code auto-derived",
        ),
        Probe(
            "wikidata_entity",
            "mapping",
            ...
            headers={"User-Agent": USER_AGENT},
            params={"language": "en"},
            note="Wikidata entity/aliases smoke",
        ),
        Probe(
            "wikidata_sparql",
            "mapping",
            ...
            params={
                "query": 'SELECT ?item ?itemLabel WHERE { ?item wdt:P31 wd:Q476028 . SERVICE wikibase:label { bd:serviceParam wikibase:language "en". } } LIMIT 3'
            },
            note="Wikidata SPARQL fair-use smoke",
        ),
        Probe(
            "open_meteo_archive",
            "weather",
            ...
            note="no-key historical weather smoke",
        ),
    ]
...
def _summary(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "warnings": counts.get("EMPTY", 0) + counts.get("RATE_LIMIT", 0) + counts.get("ENDPOINT_CONFIG", 0),
...
def _write(payload: dict[str, Any]) -> None:
    ...
    (ART_DIR / "provider-smoke-fast.json").write_text(
...
def _render_text(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        (
            f"• summary: OK {summary['ok']} | missing_secret {summary['missing_secret']} | "
            f"missing_config {summary['missing_config']} | warn {summary['warnings']} | errors {summary['errors']}"
        ),
        "",
        "📊 By group",
    ]
    for group, counts in sorted((summary or {}).get("by_group", {}).items()):
        lines.append(f"• {group}: " + ", ".join(f"{key}={value}" for key, value in sorted(counts.items())))
    lines += ["", "📡 Results"]
    for row in payload.get("results", []):
        lines.append(
            f"• [{row.get('group')}] {row['provider']}: {row['status']} | "
            f"http={row.get('http_status')} | rows={row.get('rows_count')} | {row.get('reason')}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--timeout", type=float, default=float(os.getenv("PROVIDER_SMOKE_FAST_TIMEOUT", "12")))
