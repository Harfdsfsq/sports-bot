...
def _candidate_bzzoiro_artifacts() -> list[Path]:
    names = ["latest-bzzoiro-events.json","latest-bzzoiro-odds.json","latest-bzzoiro-runtime.json","latest-bzzoiro-events-raw.json","latest-bzzoiro-odds-raw.json","latest-sstats-bzzoiro-odds-merge.json","latest-secondary-provider-matching.json","latest-signal-stack-runtime.json"]
    return [EXPORT / n for n in names if (EXPORT / n).exists()]


def _extract_events() -> tuple[list[dict[str, Any]], list[str]]:
    out: list[dict[str, Any]] = []; scanned: list[str] = []
...
def main() -> int:
    # First persist/normalize any available Bzzoiro runtime artifacts.
    try:
        from scripts.persist_bzzoiro_runtime_artifacts import main as persist
        persist()
    except Exception:
        pass
    targets = _targets(); events, scanned = _extract_events(); confirmations: list[dict[str, Any]] = []
    matched = offers = 0
    ...
    payload = {"status":"ok","created_at_utc":datetime.now(UTC).isoformat(),"targets":len(targets),"bzzoiro_events_seen":len(events),"matched_events":matched,"offers":offers,"two_source_promoted":promoted,"confirmations":confirmations[:80],"scanned_artifacts":scanned,"persistence":persist_report,"publication_contract_relaxed":False,"diagnosis":"no_bzzoiro_events_or_odds_artifact" if targets and not events else ("matched_without_offers" if matched and not offers else "ok")}
    _write(OUT, payload); print(json.dumps({k:payload[k] for k in ("targets","bzzoiro_events_seen","matched_events","offers","two_source_promoted","diagnosis")}, ensure_ascii=False)); return 0
