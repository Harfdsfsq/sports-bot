...
def patch_payload(payload:dict[str,Any])->dict[str,Any]:
    ...
    q=_load(EXPORT/'latest-a-tier-targeted-enrichment-queue.json',{}); b=_load(EXPORT/'latest-bzzoiro-targeted-odds-confirmation.json',{}); trace=_load(EXPORT/'latest-bzzoiro-report-source-trace.json',{})
...
def patch_runtime_lines(text:str)->str:
    ...
    if isinstance(trace,dict):
        persisted=_int(trace.get('persisted_event_rows')); aggregate=_int(trace.get('source_stats_events_fetched')); diag=trace.get('diagnosis') or 'n/a'
...
def patch_a_tier_summary(text:str)->str:
    q=_load(EXPORT/'latest-a-tier-targeted-enrichment-queue.json',{}); s=q.get('summary') if isinstance(q,dict) and isinstance(q.get('summary'),dict) else {}; b=_load(EXPORT/'latest-bzzoiro-targeted-odds-confirmation.json',{}); trace=_load(EXPORT/'latest-bzzoiro-report-source-trace.json',{})
    ...
    if isinstance(trace,dict) and 'Bzzoiro source trace' not in text:
        line=f"• Bzzoiro source trace: aggregate events {_int(trace.get('source_stats_events_fetched'))}; persisted rows {_int(trace.get('persisted_event_rows'))}; targeted events {_int(trace.get('targeted_events_seen'))}; diag {trace.get('diagnosis') or 'n/a'}."
...
def patch_conclusion(text:str)->str:
    repl='• Главный текущий стопор: line movement/freshness/current-price; по A-tier теперь разделяем Bzzoiro aggregate events и persisted rows для targeted matching.'
