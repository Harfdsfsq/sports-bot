...
def install(base: Any) -> dict[str, Any]:
    ...
    def hard_reject(candidate: dict[str, Any], metrics: dict[str, Any], sent_index: dict[str, Any]) -> list[str]:
        ...
        for reason in reasons:
            ...
            status = 'value_still_valid_at_current_price' if current_ev >= 4.0 and current_edge >= 2.3 else 'value_lost_after_current_price_recheck'
            metrics['current_price_recheck'] = {'selected_price': round(selected, 4), 'current_price': round(current, 4), 'model_probability_from_selected_ev': round(prob, 5), 'recalculated_ev_pct': round(current_ev, 2), 'recalculated_edge_pp': round(current_edge, 2), 'drift_pct': round(drift_pct, 2), 'status': status}
            if status == 'value_still_valid_at_current_price' and drift_pct <= 8.0:
                rechecked = True
                continue
            if status == 'value_still_valid_at_current_price':
                new_reasons.append(f'current_price_recheck_value_alive_but_drift_too_high:{current_ev:.1f}/{current_edge:.1f}')
            else:
