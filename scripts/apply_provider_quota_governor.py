...
def bzzoiro_env(grant: int) -> dict[str, str]:
    return {
        ...
        "BZZOIRO_EVENTS_MAX_REQUESTS_PER_RUN": str(1 if grant > 0 else 0),
        "BZZOIRO_PREDICTIONS_MAX_REQUESTS_PER_RUN": str(clamp_int(max(0, grant - 1), 1, 3)) if grant > 1 else "0",
        "BZZOIRO_CONTEXT_MATCH_LIMIT": str(clamp_int(8 + grant * 4, 8, 40)) if grant > 0 else "0",
        "BZZOIRO_PAGE_SIZE": "5",
